...
class ExportadorResultados:
    ...
    def _exportar_txt(
        ...
        if plantilla and not Path(plantilla).exists():
            ruta_plantilla = self._crear_plantilla_desde_formato_actual(datos, plantilla)
            print(
                "Plantilla no encontrada. "
                f"Se generó una plantilla base en: {ruta_plantilla}. "
                "Modificala y vuelve a ejecutar si querés personalizar el TXT."
            )
    ...
    def _crear_plantilla_desde_formato_actual(
        self, datos: list[dict[str, Any]], plantilla: str | None = None
    ) -> Path:
        """Crear una plantilla TXT base usando el formato tabulado actual."""
        ruta_plantilla = (
            Path(plantilla) if plantilla else self.directorio_salida / "plantilla_txt_base.txt"
        )
        ruta_plantilla.parent.mkdir(parents=True, exist_ok=True)

        cabecera = list(datos[0].keys())
        placeholders = ["{" + str(campo).upper() + "}" for campo in cabecera]
        contenido = ["\t".join(str(campo) for campo in cabecera), "\t".join(placeholders)]
    ...
    def _exportar_excel(
        ...
        if not OPENPYXL_AVAILABLE:
            self.logger.warning("openpyxl no disponible. Exportando a CSV como alternativa.")
            print(
                "⚠️  No se pudo exportar a Excel (openpyxl no disponible). "
                "Exportando a CSV en su lugar."
            )
            return self._exportar_csv(datos, nombre_archivo.replace(".xlsx", ""))

        # Escribir a Excel usando openpyxl directamente
        ...
        for col_idx, nombre_col in enumerate(columnas, 1):
            ws_datos.cell(row=1, column=col_idx, value=nombre_col)

        # Escribir datos
        for row_idx, fila in enumerate(datos, 2):  # Empezar en fila 2 (después de cabecera)
            for col_idx, columna in enumerate(columnas, 1):
                valor = fila.get(columna, "")
                try:
                    ws_datos.cell(row=row_idx, column=col_idx, value=valor)
                except Exception as exc:
                    self.logger.warning(f"Valor Excel normalizado a texto: {exc}")
                    ws_datos.cell(
                        row=row_idx,
                        column=col_idx,
                        value=self._normalizar_valor_para_excel(valor),
                    )
        ...
        for r_idx, row in enumerate(metadatos, 1):
            for c_idx, value in enumerate(row, 1):
                ws_metadatos.cell(row=r_idx, column=c_idx, value=value)
    ...
    def _normalizar_valor_para_excel(self, valor: Any) -> str:
        """Convierte valores problemáticos a texto para exportación Excel."""
        if valor is None:
            return ""
